...
module.exports = async content => {
	...
	const [related1, related2] = await Promise.all([
		...
		slots.includes['onward'] ? getRelatedContent(concepts[1], 6, content.id) : Promise.resolve([]) // get enough so that if there is an overlap pf 3 with concepts[0], there will still be some left
	])
